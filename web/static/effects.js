...
(function () {
  ...
  // crumbling rumble for Pass, a UI click for hover.
  const AudioBus = (function () {
    let ctx = null;
    let muted = true; // Muted by default to comply with browser autoplay policies, unmuted via UI toggle
    const ensure = () => {
  ...
  // ── hitChar(el, value, type) — per-card hit shake + floating number ──
  // type: 'dmg' | 'pierce' | 'afflict' | 'heal' | 'miss' | 'stun'
  function hitChar(el, value, type = 'dmg') {
    if (!el) return;
  ...
  window.JJK = { AudioBus, shake, flash, domainExpansion, tilt, attachParallax, spawnEmbers, aaa, hitChar };
})();
