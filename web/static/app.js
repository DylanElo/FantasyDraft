...
function v2MissionRoadmapHTML() {
    ...
    const liveProgress = v2State.state?.first_creation_progress;
    const account = v2State.state?.first_creation_account || FIRST_CREATION?.profile || {};
    const completedAccountIds = new Set(account.completed_missions || []);
    const ownedUnlocks = new Set(account.unlocked || []);
    const progressById = new Map((liveProgress?.missions || []).map(mission => [mission.id, mission]));
    return `
      <div class="v2-mission-head">
        <span class="stitch-label">Mission Unlocks</span>
        <strong>The apocalypse comes later</strong>
        <small>${completedAccountIds.size} mission${completedAccountIds.size === 1 ? '' : 's'} cleared · ${ownedUnlocks.size} unlock${ownedUnlocks.size === 1 ? '' : 's'} owned</small>
      </div>
      <div class="v2-mission-list">
        ${missions.slice(0, 4).map(mission => {
            const tracked = progressById.get(mission.id) || {};
            const accountComplete = completedAccountIds.has(mission.id);
            const status = accountComplete ? 'complete' : (tracked.status || 'available');
            const objectives = accountComplete
                ? (mission.objectives || []).map(label => ({ label, complete: true, current: 1, target: 1 }))
                : (tracked.objectives || (mission.objectives || []).map(label => ({ label, complete: false, current: 0, target: 1 })));
          ...
          <article class="v2-mission-card is-${esc(status)}">
            ...
            <small>Unlocks: ${(mission.unlocks || []).map(unlock => `<span class="${ownedUnlocks.has(unlock) ? 'is-owned' : ''}">${esc(unlock)}</span>`).join(' / ') || 'Progression'}</small>
          </article>`;
        }).join('')}
      </div>`;
}
...
function v2RenderResultView(state, me, foe, mine) {
    ...
    if (missionsEl) {
        ...
        const account = state.first_creation_account || FIRST_CREATION?.profile || {};
        const completed = progress?.last_completed?.length
            ...
            : [];
        const ownedDetails = account.unlock_details || [];
        missionsEl.innerHTML = completed.length
            ...
            : `<article class="v2-result-mission"><span class="material-symbols-outlined">route</span><div><strong>Mission progress saved</strong><small>${ownedDetails.length ? `${ownedDetails.filter(item => item.owned).length} rewards owned on this profile.` : 'Try a recommended team to unlock the next route.'}</small></div></article>`;
    }
}
