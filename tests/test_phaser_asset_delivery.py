...
const current = await import('./web/static/phaser/ui/culling-current-ui.js?v=36');
const flow = await import('./web/static/phaser/ui/season-three-ui.js?v=36');
const post = await import('./web/static/phaser/ui/season3-master-ui.js?v=36');
console.log(JSON.stringify({
...
def test_runtime_cache_chain_and_delivery_manifest_agree_on_v36():
    budget = json.loads((ASSET_ROOT / "runtime-texture-budget.json").read_text(encoding="utf-8"))
    ...
    assert budget["runtime_cache_version"] == "36"
    assert "const SHELL_VERSION = '36';" in shell
    assert "phaser-shell.js') }}?v=36" in template
    assert "phaser-design-tokens.js') }}?v=36" in template
    ...
    for path in (ROOT / "web" / "static" / "phaser").rglob("*.js"):
        ...
        for specifier in specifier_pattern.findall(source):
            if specifier.startswith(".") and not specifier.endswith("?v=36"):
