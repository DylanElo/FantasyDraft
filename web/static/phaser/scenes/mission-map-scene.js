...
import { clamp, shortText, titleize } from '../core/text.js?v=16';
...
export class MissionMapScene extends BaseScene {
    ...
    renderMissionCard(mission, x, y, w, h, index) {
      const tone = index === 0 ? COLORS.gold : COLORS.purple;
      this.cardPanel(x, y, w, h, tone, 0.74);
      this.mono(x + 14, y + 12, titleize(mission.tier || 'starter').toUpperCase(), { color: '#fde68a', fontSize: '8px' });
      ...
      (mission.objectives || []).slice(0, 2).forEach((objective, objectiveIndex) => {
        this.mono(x + 16, y + 82 + objectiveIndex * 18, `- ${shortText(objective, 44)}`, { color: '#94a3b8', fontSize: '8px' });
      });
      this.mono(x + 14, y + h - 51, 'RECOMMENDED TEAM', { color: '#fde68a', fontSize: '8px' });
    ...
    renderLockedRoutes(frame, y) {
      ...
      this.mono(x + 14, y + 12, 'LOCKED ROUTES', { color: '#94a3b8', fontSize: '8px' });
      ['Shibuya Incident', 'Culling Game', 'Shinjuku Showdown'].forEach((route, index) => {
        ...
        this.mono(rx + 8, y + 47, shortText(route, 13), { color: '#64748b', fontSize: '7px' });
      });
    }
    ...
    render() {
      ...
      let y = 92;
      this.cardPanel(x, y, frame.width - 32, 74, COLORS.gold, 0.66);
      this.text(x + 16, y + 13, 'Student Era Route', {
        fontFamily: 'Cinzel, Inter, serif',
        fontSize: '18px',
        fontStyle: '900',
      });
      this.mono(x + 18, y + 44, 'Clear starter missions to reveal later arcs.', { color: '#cbd5e1', fontSize: '9px' });
      y += 94;
      ...
      this.button(x, frame.height - 106, 78, 34, 'Prev', () => {
        ...
        this.store.notify();
      }, { disabled: this.store.missionPage === 0, fill: 0x111827, mono: true, fontSize: '9px' });
      this.mono(x + 94, frame.height - 96, `Route ${this.store.missionPage + 1}/${maxPage + 1}`, { color: '#94a3b8', fontSize: '9px' });
      this.button(x + frame.width - 108, frame.height - 106, 76, 34, 'Next', () => {
        ...
        this.store.notify();
      }, { disabled: this.store.missionPage === maxPage, fill: 0x111827, mono: true, fontSize: '9px' });
