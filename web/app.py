...
def on_battle_v2_resume(data=None):
    ...
    if not valid:
        ...
        return
    try:
        # Prove the authoritative reconnect can produce this viewer's state first.
...
def on_reset_room():
    ...
    if not room_id:
        return
    remove_battle_v2_room(room_id)
    for entry in list(v2_pvp_lobbies.get(room_id, [])):
        remove_v2_pvp_lobby_player(room_id, entry["id"])
    session.pop("room_id", None)
    session.pop("match_id", None)
    emit("room_reset", {}, room=lobby_room(room_id))
