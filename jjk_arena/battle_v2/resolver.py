...
def finish_turn(state: BattleState, player_id: str) -> list[BattleEvent]:
    ...
    for player in state.players.values():
        for slot, character in enumerate(player.team):
            statuses_before_tick = list(character.statuses)
            replacements_before_tick = dict(character.skill_replacements)
            ...
            remaining_status_objects = {id(status) for status in character.statuses}
            for expired in statuses_before_tick:
                if id(expired) in remaining_status_objects:
                    continue
                payload = {"status": expired.id, "target_player_id": player.id, "target_slot": slot}
                if SkillClass.DOMAIN in expired.classes or expired.payload.get("domain"):
                    _append_event(events, state, BattleEvent("domain_ended", f"{expired.name} ended", state.turn_number, payload))
                elif expired.source_player_id != expired.target_player_id:
                    _append_event(events, state, BattleEvent("hostile_status_expired", f"{expired.name} expired", state.turn_number, payload))
            for base_skill, replacement in replacements_before_tick.items():
                if character.skill_replacements.get(base_skill) != replacement:
                    _append_event(events, state, BattleEvent(
                        "replacement_revoked",
                        f"{replacement} was revoked",
                        state.turn_number,
                        {"target_player_id": player.id, "target_slot": slot, "base_skill_id": base_skill, "replacement_skill_id": replacement},
                    ))
...
def finish_match(
    ...
    *,
    ...
    """Apply every terminal result through one normalized state transition."""

    normalized = str(result_type).upper()
    state.winner_id = winner_id
    state.result_type = normalized
    state.result_reason = str(reason)
    ...
    event = BattleEvent(
        ...
        message=f"{normalized}: {state.result_reason}",
        turn_number=state.turn_number,
...
def event_is_meaningful_progress(event: BattleEvent) -> bool:
    """Classify state-changing events for no-progress accounting."""

    if event.type in {"energy_drained", "energy_stolen", "status_consumed", "status_stack_changed"}:
        return int(event.payload.get("amount", 0) or 0) > 0
    if event.type in {"status_duration_ticked", "energy_drain_skipped"}:
        return False
    return event.type in {
        "damage", "heal", "health_stolen", "character_defeated", "status_applied",
        "status_removed", "hostile_status_expired", "domain_started", "domain_ended",
        "replacement_granted", "replacement_revoked", "battle_finished",
    }
...
def check_winner(state: BattleState) -> list[BattleEvent]:
    ...
    if len(alive_players) == 1:
        event = finish_match(
            ...
            reason="all opposing active characters were defeated",
        )
        return [event]
    return []
