...
export class LobbyScene extends BaseScene {
    ...
    render() {
      ...
      let y = 92;
      this.cardPanel(x, y, frame.width - 32, 142, COLORS.purple, 0.88);
      const heroCx = x + frame.width - 92;
      const heroCy = y + 70;
      [52, 36, 20].forEach((radius, index) => {
        this.graphics.lineStyle(index === 1 ? 2 : 1, index === 1 ? COLORS.gold : COLORS.purple, index === 1 ? 0.48 : 0.28);
        this.graphics.strokeCircle(heroCx, heroCy, radius);
      });
      this.graphics.lineStyle(1, COLORS.gold, 0.28);
      ...
      this.mono(x + 20, y + 52, 'CURSED CLASH / FIRST CREATION', { color: '#c4b5fd', fontSize: '9px' });
      this.mono(x + 20, y + 76, 'Draft a trio. Break the domain.', { color: '#fde68a', fontSize: '10px' });
      ...
      }, { fill: 0x161b2f, stroke: COLORS.purple, fontSize: '11px', mono: true, radius: 14 });
      ...
      }, { fill: 0x161b2f, stroke: COLORS.cyan, fontSize: '11px', mono: true, radius: 14 });

      y += 162;
      this.button(x, y, frame.width - 32, 82, 'Quick Play', () => {
        this.store.setMatchMode('cpu');
        this.store.changeScene('DraftScene');
      }, { fill: 0x182033, gradientTop: 0x24324f, stroke: COLORS.gold, fontSize: '20px', radius: 18, glowAlpha: 0.18 });
      this.talismanLabel(x + 16, y + 50, 'CPU PRACTICE', COLORS.gold);
      ...
      this.button(x, y, modeW, 64, 'Private PvP', () => {
        ...
        this.store.changeScene('DraftScene');
      }, { fill: 0x081f2b, gradientTop: 0x10233b, stroke: COLORS.cyan, fontSize: '15px', radius: 18 });
      this.button(x + modeW + 12, y, modeW, 64, 'First Creation', () => {
        this.store.setMatchMode('cpu');
        this.store.changeScene('FirstCreationScene');
      }, { fill: 0x1a1224, gradientTop: 0x2b1744, stroke: COLORS.purple, fontSize: '14px', radius: 18 });
      this.talismanLabel(x + 16, y + 38, 'PRIVATE ROOM', COLORS.cyan);
      this.talismanLabel(x + modeW + 28, y + 38, 'STARTER TRIO', COLORS.purple);

      y += 82;
      const half = (frame.width - 44) / 2;
      this.button(x, y, half, 54, 'Mission Map', () => this.store.changeScene('MissionMapScene'), { fill: 0x101827, stroke: COLORS.gold, fontSize: '12px' });
      this.button(x + half + 12, y, half, 54, 'Records', () => this.store.changeScene('RecordsScene'), { fill: 0x101827, stroke: COLORS.green, fontSize: '12px' });
