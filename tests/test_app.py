...
def test_index_exposes_battle_v2_entry_when_enabled(monkeypatch):
    ...
    assert 'phaser-design-tokens.js?v=32' in html
    assert 'phaser-shell.js?v=32' in html
    assert 'phaser-shell.css?v=32' in html
...
def test_battle_v2_public_surface_uses_production_copy(monkeypatch):
    ...
    assert "const SHELL_VERSION = '32';" in shell_js
    assert "import './legacy-shell.js?v=32';" in phaser_entry_js
    assert "from './store/game-store.js?v=32';" in runtime_js
    assert "from './network/socket-client.js?v=32';" in runtime_js
    assert "from './scenes/scene-registry.js?v=32';" in runtime_js
    assert "scene: SCENE_LIST" in runtime_js
    assert "from './scenes/boot-scene.js?v=32';" not in runtime_js
    assert "from './boot-scene.js?v=32';" in scene_registry_js
    ...
    assert "from './matchup-scene.js?v=32';" in scene_registry_js
