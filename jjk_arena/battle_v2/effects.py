...
def apply_effect(
    ...
    *,
    ...
    if effect.type == "apply_status":
        ...
        previous_stack = next((active.stacks for active in target.statuses if active.duration != 0 and active.id == effect.status), None)
        ...
        stack_delta = status.stacks - previous_stack if previous_stack is not None else 0
        return BattleEvent(
            type="status_stack_changed" if previous_stack is not None and stack_delta != 0 else "status_applied",
